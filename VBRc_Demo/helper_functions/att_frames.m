...
function att_frames()
    ...
    freqs = logspace(-13,1, 25);
    for ifreq = 1:numel(freqs)
        disp(ifreq)
        disp(sprintf('%05d', num2str(ifreq)))
        n = 42;
        frame_str = sprintf('%05d', ifreq);
        build_frame(freqs(ifreq), 1400, ['frames/att_map_', frame_str, '.png'])
    end

end

function build_frame(freq, T_C, fname)
